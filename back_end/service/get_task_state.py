...
async def get_task_state_wx(tag_task_id: str, mongo_db: AsyncIOMotorDatabase) -> str:
    """
    获取并刷新task状态信息
    :param mongo_db: mongo数据库
    :param tag_task_id: tag任务id
    :return:
    """
    mongo_collection = mongo_db['wx_task']
    tag_task_result = await mongo_collection.find_one({'tag_task_id': tag_task_id})
    if tag_task_result['status'] != 'SUCCESS':
        if tag_task_result['status'] == 'FAILURE':
            return tag_task_result['status']
...
async def get_task_state_rm(tag_task_id: str, mongo_db: AsyncIOMotorDatabase) -> str:
    """
    获取并刷新task状态信息
    :param mongo_db: mongo数据库
    :param tag_task_id: tag任务id
    :return:
    """
    mongo_collection = mongo_db['rm_task']
    tag_task_result = await mongo_collection.find_one({'tag_task_id': tag_task_id})
    if tag_task_result['status'] != 'SUCCESS':
        if tag_task_result['status'] == 'FAILURE':
            return tag_task_result['status']
...
async def get_task_state_tb(tag_task_id: str, mongo_db: AsyncIOMotorDatabase) -> str:
    """
    获取并刷新task状态信息
    :param mongo_db: mongo数据库
    :param tag_task_id: tag任务id
    :return:
    """
    mongo_collection = mongo_db['tb_task']
    tag_task_result = await mongo_collection.find_one({'tag_task_id': tag_task_id})
    if tag_task_result['status'] != 'SUCCESS':
        if tag_task_result['status'] == 'FAILURE':
            return tag_task_result['status']
