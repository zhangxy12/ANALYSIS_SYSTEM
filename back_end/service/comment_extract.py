...
async def deleteTask_tb(tag_task_id, mongo_db: AsyncIOMotorDatabase):
    result = mongo_db[mongo_conf.TB_COMMENT_TASK].find({'tag_task_id': tag_task_id})
    comment_task_id_list = list()
    celery_task_id_list = list()
    for i in await result.to_list(length=10):
        comment_task_id_list.append(i['tag_comment_task_id'])
        celery_task_id_list.append(i['celery_id'])
    await mongo_db[mongo_conf.TB_COMMENT_TASK].delete_many({'tag_task_id': tag_task_id})
    for comment_task_id, celery_task_id in zip(comment_task_id_list, celery_task_id_list):
        query = {'tag_comment_task_id': comment_task_id}
        await mongo_db[mongo_conf.TB_COMMENT_CLOUD].delete_one(query)
        await mongo_db[mongo_conf.TB_COMMENT_IP].delete_one({'tag_task_id': tag_task_id})
        await mongo_db[mongo_conf.TB_COMMENT_MOOD].delete_one(query)
        # await mongo_db[mongo_conf.COMMENT_REPOSTS].delete_many(query)
        # await mongo_db[mongo_conf.COMMENT_TENDENCY].delete_one(query)
        # await mongo_db[mongo_conf.COMMENT_TREE].delete_one(query)
        celeryapp.control.revoke(celery_task_id, terminate=True, signal=signal.CTRL_C_EVENT)
...
# 贴吧
async def get_comment_task_id_tb(tag_task_id: str, url: str, mongo: AsyncIOMotorDatabase):
    """
    以tag_task_id 获取评论 id
    :param tag_task_id:
    :param url:
    ...
    :return:
    """
    comment_task_collection = mongo[mongo_conf.TB_COMMENT_TASK]
    comment_task = await comment_task_collection.find_one({'tag_task_id': tag_task_id, 'url': url})
    if comment_task:
        return comment_task.get('tag_comment_task_id')

async def getTbById(tag_task_id: str, url: str, mongo_db: AsyncIOMotorDatabase):
    """
    以id获得tb详细内容
    :param tag_task_id: 话题任务id
    :param tb_id: tbid
    ...
    :return:
    """
    result = await mongo_db[mongo_conf.TB_COMMENT_TASK].find_one({"tag_task_id": tag_task_id, "url": url})

    if result:
        return result['detail']
    return None

async def getIPById(comment_task_id: str, mongo_db: AsyncIOMotorDatabase):
    result = await mongo_db[mongo_conf.TB_COMMENT_IP].find_one({"tag_comment_task_id": comment_task_id})
    print(result)
    if result:
        result.pop("_id")
    return result

async def getByCloudId_tb(comment_task_id: str, mongo_db: AsyncIOMotorDatabase):
    """
    以 tag_comment_task_id 获取词云内容
    :param comment_task_id: tag_comment_task_id
    :param mongo_db: mongo
    :return:
    """
    item = await mongo_db[mongo_conf.TB_COMMENT_CLOUD].find_one({"tag_comment_task_id": comment_task_id})
    if item:
        item.pop("_id")
    return item

async def getByMood_tb(comment_task_id: str, mongo_db: AsyncIOMotorDatabase):
    """
    以 tag_comment_task_id 获取情感分析内容
    :param comment_task_id: tag_comment_task_id
    :param mongo_db: mongo
    :return:
    """
    item = await mongo_db[mongo_conf.TB_COMMENT_MOOD].find_one({"tag_task_id": comment_task_id})
    print(item)
    if item:
        item.pop("_id")
    return item
