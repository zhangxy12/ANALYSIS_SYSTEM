...
HEADERS = {
    ...
    'Cookie': 'SCF=AsrlXm48SwY-ZJCGDkF1GT4qOPp_tYe8x5_NUBdx_CdERDFMdWVE9pVqYGCSLKoxSSCUEnGFubX_qV-B6bikFvE.; SUB=_2A25KU9hbDeRhGeBO61QV9SnMwjmIHXVpEVWTrDV6PUJbktAYLRLXkW1NSlJXOClXHwr6GFvFeLb7hSJq4R4Qp4oZ; SUBP=0033WrSXqPxfM725Ws9jqgMF55529P9D9Wh88SPkU_j3IuSZpgLkJnYD5NHD95Qceh5cSh-Neh.fWs4Dqcj.i--Ni-iFi-zRi--Ni-ihi-8Wi--ci-zfiKnfi--Ni-8hiK.p; SSOLoginState=1733797899; ALF=1736389899; _T_WM=db40324907dab26f77e8f39979110369'
 }
...
def spider(tag_task_id: str, weibo_id, page=93, tag_comment_task_id=9999):
    """
    爬取微博评论函数
...
def spider_list(self, tag_task_id: str, weibo_id, tag_comment_task_id=9999):
    ...
    if all_page:
        ...
        all_page = 10
    page_data = list()
    print("Data to insert:", page_data)  # 输出待插入数据
    if page_data:
        print("Data to insert:", page_data)  # 输出待插入数据
...
def spider_list_part(tag_task_id: str, weibo_id, tag_comment_task_id=9999):
    print(weibo_id)
    response = requests.get("{domain}/repost/{weibo_id}?page=1".format(domain=BASE_DOMAIN, weibo_id=weibo_id),
                            headers=HEADERS,verify=False)
    all_page = re.search(r'/>&nbsp;1/(\d+)页</div>', response.text)
