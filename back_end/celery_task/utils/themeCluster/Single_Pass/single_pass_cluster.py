...
class SinglePassCluster():
    def __init__(self, blog_data: list, tag_task_id: str, user_list: list,stopWords_path="D:\\dasanshang\\NLP\\1\\Topic_and_user_profile_analysis_system\\code\\back_end\\dict\\哈工大停用词表.txt", my_stopwords=None,
    ...
    def single_pass(self):
        ...
        try:
            result = mongo_client.db[mongo_conf.USER].update_one( {
                '$set': update
            }, upsert=True)
...
if __name__ == "__main__":
    data = list()
    blog_post = mongo_client.db[mongo_conf.BLOG].find_one({'tag_task_id': '422869aaeadfaafcd27bd1834102d67a'}).get('data')
    ...
    user_list = mongo_client.db[mongo_conf.USER].find_one({'tag_task_id': '422869aaeadfaafcd27bd1834102d67a'}).get('data')
    ...
    cluster = SinglePassCluster(max_features=100, blog_data=blog_post, user_list=user_list, tag_task_id='422869aaeadfaafcd27bd1834102d67a',
                                simi_threshold=0.1)
    cluster.single_pass()
