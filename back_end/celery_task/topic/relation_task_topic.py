...
def topic_tag_relation(weibo_data: dict, topic: str, user_mark_data: dict):
    """
    处理话题人物关系网的函数
            ...
            if i not in screen_name_set:
                ...
                for node_item in node_list:
                    if node_item['name'] == i:
                        ...
                        break
    query_by_task_id = {'topic': topic}
    update = {
        "$set": {'nodes_list': node_list, 'links_list': link_list, 'categories': user_mark_data.get('categories')}}
    mongo_client.db[mongo_conf.TOPIC_RELATION].update_one(query_by_task_id, update, upsert=True)
    mongo_client.db[mongo_conf.TOPIC_CHARACTER].update_one(query_by_task_id, {'$set': {'detail': node_list}},
                                                           upsert=True)
