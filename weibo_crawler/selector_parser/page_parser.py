        ...
        try:
            ...
            for a in a_list:
                if ('place.weibo.com' in a.xpath('@href')[0]
                    ...
                    weibo_a = div_first.xpath("span[@class='ct']/a")
                    if len(weibo_a) >= 1:
                        publish_place = weibo_a[-1]
                        if (u'视频' == div_first.xpath(
                                "span[@class='ct']/a/text()")[-1][-2:]):
                            if len(weibo_a) >= 2:
        ...
        try:
            ...
            if first_pic in all_href:  # 检查是否有单张的缩略图
                if all_pic in all_href:  # 检查该条微博是否有多图
                    ...
                    if mblogPicAllParser is not None:
                        ...
                        LOGGING.warning(
                            "mblogPicAllParser is None, unable to extract preview picture list for multi-picture微博.")
                else:
    ...
    def _parse_one_comment(node):
        ...
        for span_node in span_nodes:
            ...
            if klass == 'kt':
                ...
                # 获取发布位置
                if '来自' in str_time:
                    location = str_time.split('来自')[1]  # 提取位置部分
                    comment['location'] = location.strip()
                else:
                    comment['location'] = None  # 如果没有找到来自信息，则设置为 None
