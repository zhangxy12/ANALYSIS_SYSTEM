    ...
    def _parse_weibo(self):
        ...
        for sel in selector.xpath("//div[@class='card-wrap']"):
            ...
            if len(info) != 0:
                ...
                # 先按当前结构查找
                weibo_id_xpath = sel.xpath('(.//div[@class="from"])[last()]/a[1]/@href')
                if not weibo_id_xpath:
                    # 若按结构没找到，用通用特征找
                    weibo_id_xpath = sel.xpath('//a[contains(@href,"weibo.com") and contains(@href,"/")]/@href')

                if weibo_id_xpath:
                    href = weibo_id_xpath[0]
                    weibo['weibo_id'] = href.split('/')[-1].split('?')[0]
                else:
                    weibo['weibo_id'] = '获取失败'
                ...
                # 获取创建时间的 XPath 表达式
                created_at_xpath = sel.xpath('.//div[@class="from"]/a/text()')

                # 检查返回结果
                if created_at_xpath:
                    created_at = created_at_xpath[0].replace(' ', '').replace('\n', '').strip()
                    weibo['created_at'] = utils.standardize_date(created_at)
                else:
                    weibo['created_at'] = '获取失败'  # 或者设置为其他默认值，如 None 或空字符串
