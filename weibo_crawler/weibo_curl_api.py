...
class SearchTweetsHandler(BaseHandler):
    ...
    def get(self):
        ...
        keyword, cursor, is_hot = args_dict.get('keyword'), args_dict.get(
            'cursor', '1'), args_dict.get('is_hot', True)
        if keyword is None:
...
class StatusesShowHandler(BaseHandler):
    ...
    def get(self):
        ...
        if weibo_id is None:
            ...
            return
        hot = args_dict.get('hot', True)  # 是否获取热评
        ...
        if not hot:
            ...
            print("weibo_detail['max_page'] ", weibo_detail['max_page'])
            comment_list = []
            for page in range(1, weibo_detail['max_page'] + 1):
                hot_comment_curl_result = yield weibo_web_curl(SpiderAim.hot_comment, weibo_id=weibo_id, page_num=page)
                print("评论page=", page)
                if not hot_comment_curl_result['error_code']:
                    ...
                    return
                try:
                    comment_list.extend(HotCommentParser(weibo_id, self.hot_comment_response).get_all_comment())
                except HTMLParseException:
                    self.write(WeiboCurlError.HTML_PARSE_ERROR)
                    return
                except Exception as e:
...
class FriendsHandler(BaseHandler):
    ...
    def get(self):
        ...
        try:
            ...
            return
        # 进行爬取
        follow_curl_result = yield SpiderAim(SpiderAim.follow, user_id=user_id, page_num=cursor)
