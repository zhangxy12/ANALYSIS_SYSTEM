...
import chardet  # 导入 chardet 用于检测编码
    ...
    for epoch in range(retry_time):  # 最多进行retry_time次的请求尝试
        ...
        try:
            ...
            try:
                ...
                charset_match = charset_pattern.search(decoded_content)
                if charset_match:
                    charset = charset_match.group()  # 获取该网页的编码方案
                else:
                    # 如果没有找到，则尝试使用 chardet 自动检测编码
                    detected_encoding = chardet.detect(response.body)['encoding']
                    charset = detected_encoding if detected_encoding else 'utf-8'

                # 输出调试信息
                # print(decoded_content)
                # 用于寻找html中title部分的正则匹配pattern
                title_pattern = re.compile(r'<title>.*</title>')
                html_title = title_pattern.search(decoded_content).group(0)
