<template>
    <div class="default_panel">
      ...
      <div v-if="topics_show" class="topics" style="height: 70%;">
        <div
          class="topic"
        ...
        >
          ...
          <div class="other_info" v-if="!defaultInfo.status">
            用户数量:{{ defaultInfo.tb_user }} 文章数量:{{ defaultInfo.tb_count }}
          </div>
  ...
  export default {
    ...
    methods: {
      requesrInfo() {
        this.$axios.get("/tag_list_tb").then((res) => {
      ...
      deleteTopic(id,index){
        this.$confirm('此操作将删除该话题, 是否继续?', '提示', {
          ...
          this.$axios.get('delete_task_tb?tag_task_id=' + id).then((res) =>{
            if(res.data.code == 0){
