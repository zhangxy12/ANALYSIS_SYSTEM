<template>
    <div class="default_panel">
      ...
      <div class="default_panel_top" style="height: 30px;">
        <span>
          <svg class="icon" aria-hidden="true">
      ...
      <div v-if="topics_show" class="topics" style="height: 79%;">
        <div
          class="topic"
        ...
        >
          ...
          <div class="other_info" v-if="!defaultInfo.status">
            来源数量:{{ defaultInfo.rm_origin }} 文章数量:{{ defaultInfo.rm_count }}
          </div>
  ...
  export default {
    ...
    methods: {
      requesrInfo() {
        this.$axios.get("/tag_list_rm").then((res) => {
      ...
      deleteTopic(id,index){
        this.$confirm('此操作将删除该话题, 是否继续?', '提示', {
          ...
          this.$axios.get('delete_task_rm?tag_task_id=' + id).then((res) =>{
            if(res.data.code == 0){
